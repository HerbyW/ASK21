...
var stalltimer = maketimer(3.0, func {
  var airspeed = getprop("/velocities/airspeed-kt");
  var alt=getprop("/position/gear-agl-m");
  
  if (airspeed > 37.8 or alt < 10)
  {
  ...
  else
  {
    ...
    if (airspeed > 35.6 and alt > 10)
    {
      interpolate("/controls/flight/stallflap", 0.02, 2.8);
      
    }
    else
    {
      
      if (airspeed > 34 and alt > 10)
      {
	interpolate("/controls/flight/stallflap", 0.2 , 2.8);
	
      }
      else
      {
	
	if (airspeed > 30 and alt > 10)
	{
